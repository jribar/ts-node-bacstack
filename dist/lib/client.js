...
class Client extends events_1.EventEmitter {
    ...
    _processConfirmedServiceRequest(address, type, service, maxSegments, maxApdu, invokeId, buffer, offset, length) {
        ...
        if (service === baEnum.ConfirmedServiceChoice.READ_PROPERTY) {
            ...
            if (!result)
                return debug('Received invalid readProperty message');
            ...
            if (!result)
                return debug('Received invalid writeProperty message');
            ...
            if (!result)
                return debug('Received invalid covNotify message');
            ...
            if (!result)
                return debug('Received invalid atomicReadFile message');
            ...
            if (!result)
                return debug('Received invalid subscribeCOV message');
            ...
            if (!result)
                return debug('Received invalid deviceCommunicationControl message');
            ...
            if (!result)
                return debug('Received invalid readRange message');
            ...
            if (!result)
                return debug('Received invalid createObject message');
            ...
            if (!result)
                return debug('Received invalid deleteObject message');
            ...
            if (!result)
                return debug('Received invalid addListElement message');
    ...
    _processUnconfirmedServiceRequest(address, type, service, buffer, offset, length) {
        ...
        if (service === baEnum.UnconfirmedServiceChoice.I_AM) {
            ...
            if (!result)
                return debug('Received invalid iAm message');
            /**
            ...
            if (!result)
                return debug('Received invalid WhoIs message');
            /**
            ...
            if (!result)
                return debug('Received invalid WhoHas message');
            ...
            if (!result)
                return debug('Received invalid TimeSync message');
            /**
            ...
            const result = baServices.timeSync.decode(buffer, offset);
            if (!result)
                return debug('Received invalid TimeSyncUTC message');
            /**
            ...
            const result = baServices.eventNotifyData.decode(buffer, offset);
            if (!result)
                return debug('Received invalid EventNotify message');
            ...
            if (!result)
                return debug('Received invalid ihaveBroadcast message');
    ...
    _handleNpdu(buffer, offset, msgLength, remoteAddress) {
        // Check data length
        if (msgLength <= 0)
            return debug('No NPDU data -> Drop package');
        // Parse baNpdu header
        ...
        if (msgLength <= 0)
            return debug('No APDU data -> Drop package');
